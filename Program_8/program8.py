...
sample_data=pd.DataFrame({
    ...
    'joboffer':['yes','yes','no','no','yes','yes','no','yes','yes','yes']
})

label_encoders={}
for column in ['interactiveness','practical_knowledge','communication','joboffer']:
  le=LabelEncoder()
  sample_data[column]=le.fit_transform(sample_data[column])
  label_encoders[column]=le

X_sample=sample_data.drop('joboffer',axis=1)
y_sample=sample_data['joboffer']

sample_model=DecisionTreeClassifier(max_depth=4,random_state=42)
sample_model.fit(X_sample,y_sample)

plt.figure(figsize=(12,6))
plot_tree(sample_model,feature_names=X_sample.columns,class_names=label_encoders['joboffer'].classes_,filled=True,rounded=True)
...
predicted_label=label_encoders['joboffer'].inverse_transform(prediction)
